...
class ActiveRecords
{
     ...
     * *    Output : SELECT * FROM table_name WHERE BINARY col_1=val_1 AND BINARY col_2=val2
     * *
     * * 4. Code   : $obj->select('table_name', $array, 'OR');
     * *    Output : SELECT * FROM table_name WHERE BINARY col_1=val_1 OR BINARY col_2=val2
     *
     * @param string $table
     ...
     * *    Output : SELECT * FROM table_1 JOIN table_2 ON table_1.col_name=table_2.col_name
     * *             WHERE BINARY col_1=val_1 AND BINARY col_2=val2;
     *
     * * 5. Code   : $obj->join('table_1', 'table_2', 'table_1.col_name=table_2.col_name', $array, 'OR');
     * *    Output : SELECT * FROM table_1 JOIN table_2 ON table_1.col_name=table_2.col_name
     * *             WHERE BINARY col_1=val_1 OR BINARY col_2=val2;
     *
     * @param mixed  $select
     * @param mixed  $join
     * @param string $on
    ...
    public function join($select, $join, $on, $condition = [], $glue = 'AND')
    {
        ...
        if (empty($condition)) {
            ...
            foreach ($condition as $key => $value) {
                $pieces[] = 'BINARY '.$key.'=:'.str_replace('.', '_', $key);
            }
            $where = ' WHERE '.implode(' '.trim($glue).' ', $pieces);
        }
    ...
    public function update($table, $values, $condition = [], $glue = 'AND')
    {
        ...
        if (empty($condition) !== true) {
            foreach ($condition as $key => $value) {
                $wherePieces[] = 'BINARY '.$key.'=:where'.$key;
            }
    ...
    public function dropTables()
    {
        if ($this->__construct() === true) {
            ...
            $query->execute();
            $array = $query->fetchAll(\PDO::FETCH_BOTH);
